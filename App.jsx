...
export default function App() {
  ...
  const handleManualAssign = (taskId, soldierId) => {
    setAssignments(prev => {
  ...
  const handleAddRoutineTask = (task) => {
    const newTask = {
      ...task,
  ...
  const handleUpdateSoldier = (updatedSoldier) => {
    setSoldiers(prev => prev.map(s => s.id === updatedSoldier.id ? updatedSoldier : s));
  };

  const handleToggleAtHome = (soldierId) => {
  ...
  const handleRemoveConstraint = (constraintId) => {
  ...
  const handleToggleRoutineTask = (taskId) => {
  ...
  return (
    <div className="app-layout" dir="rtl">
      ...
      {/* ─── SIDEBAR ─── */}
      <aside className={`app-sidebar ${!isSidebarOpen ? 'sidebar-closed' : ''}`}>
        {/* Branding */}
              ...
              <div className="premium-card">
                ...
                <div className="card-body">
                  <div className="routine-tasks-big-list">
                    <AnimatePresence>
                      {todayTasks.filter(t => !t.isDone).map(t => {
                      ...
                      {todayTasks.filter(t => !t.isDone).length === 0 && (
                        <motion.div
                          initial={{ opacity: 0 }}
                          animate={{ opacity: 1 }}
